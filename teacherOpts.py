...
class TeacherOptions():
    ...
    def topicDes(self, root, topicTitle, topicFile):
        ...
        defLabel.place(x = 0, y = 200, width = 1250)

        editLabel = tk.Label(topicPage, text = "Add to topic:", bg = "black", fg = "yellow", bd = 16, font = 'Times 16 bold italic', relief = tk.RAISED)
        editLabel.place(x = 50, y = 500, width = 200)
        
        editEntry = tk.Entry(topicPage, bg = "black", fg = "yellow", font = "Times 20 bold italic")
        editEntry.place(x = 250, y = 500, width = 600)

        editButton = tk.Button(topicPage, text = "Add", bg = "black", fg = "yellow", font = "Times 16 bold italic", relief = tk.RAISED, command = lambda: self.editFile(topicFile, editEntry.get()))
        editButton.place(x = 850, y = 500)
        
        backBtn = tk.Button(topicPage, text = "BACK", bg = "black", fg = "yellow", bd = 16, font = "Times 16 bold italic", relief = tk.RAISED, command = lambda: self.delete(classPage))
        backBtn.place(x = 500, y = 600)

    
    def editFile(self, filename, text):
